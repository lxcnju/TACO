...
def construct_loaders(args):
    if args.dataset == "MiniImageNet":
        ...
        from data import MiniImagenetUnlabelDataset as UnDataset
    else:
    ...
    unlabel_train_set = UnDataset(
        unlabel_setname=args.unlabel_train_set,
        read_mem=args.read_mem,
        backbone=args.backbone,
        rand_aug=args.rand_aug,
        is_train=True,
    ...
    unlabel_train_loader = DataLoader(
        dataset=unlabel_train_set,
        batch_size=args.n_unlabel,
        num_workers=args.num_workers,
        pin_memory=True,
        shuffle=True
    )
...
def train(model, train_loader, unlabel_train_loader, optimizer, args):
    model.train()
    avg_loss = Averager()
    avg_acc = Averager()
    ...
    for i, batch in enumerate(train_loader, 1):
        ...
        try:
            unlabel_data = unlabel_train_loader_iter.next()
        except Exception:
            unlabel_train_loader_iter = iter(unlabel_train_loader)
            unlabel_data = unlabel_train_loader_iter.next()
        ...
        unlabel_data = model.encoder(unlabel_data)
        ...
        # TACO
        un_logits_1 = model.logits(data_shot_1, unlabel_data)
        un_logits_2 = model.logits(data_shot_2, unlabel_data)

        # TACO
        kl_loss_1 = F.kl_div(
            F.log_softmax(un_logits_1, dim=1),
            F.softmax(un_logits_2.detach() / args.kl_tau, dim=1),
            reduction="batchmean"
        )
        kl_loss_2 = F.kl_div(
            F.log_softmax(un_logits_2, dim=1),
            F.softmax(un_logits_1.detach() / args.kl_tau, dim=1),
            reduction="batchmean"
        )
...
def main(para_dict):
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        train_loss, train_acc = train(
            model, train_loader, unlabel_train_loader,
            optimizer, args
        )
        val_acc = test(
...
if __name__ == "__main__":
    ...
    candi_param_dict = {
        ...
        "pretrain": [True],
        "rand_aug": [False],
        "train_set": ["train_label_0.3_C"],
        "val_set": ["val_label_0.3_C"],
        "unlabel_train_set": ["train_unlabel_0.3_C"],
        "unlabel_val_set": ["val_unlabel_0.3_C"],
        "test_set": ["test"],
        "n_train_task": [100],
        "n_val_task": [100],
        "n_test_task": [1000],
    ...
    set_gpu(args.gpu)
    for backbone in ["ConvNet", "ResNet"]:
        for set_type in ["C", "I"]:
            train_set = "train_label_0.3_{}".format(set_type)
            unlabel_train_set = "train_unlabel_0.3_{}".format(set_type)
            val_set = "val_label_0.3_{}".format(set_type)
            unlabel_val_set = "val_unlabel_0.3_{}".format(set_type)
                        ...
                        for kl_tau in kl_taus:
                            ...
                            for _ in range(1):
                                ...
                                for k, vs in candi_param_dict.items():
                                    ...
                                    para_dict["train_set"] = train_set
                                    para_dict["unlabel_train_set"] = unlabel_train_set
                                    para_dict["val_set"] = val_set
                                    para_dict["unlabel_val_set"] = unlabel_val_set
                                    para_dict["n_shot"] = n_shot
                                    para_dict["dis_tau"] = dis_tau
